...
import { Banknote, CheckCircle, Clock, CreditCard, DollarSign, Home, Landmark, PlusCircle, Scale, UserCheck, Zap, Printer } from "lucide-react";
import { PieChart, Pie, Cell, ResponsiveContainer } from 'recharts';
import { format } from "date-fns";

// Datos de Ejemplo para el Puntaje
const PUNTAJE_CREDITICIO = 806;
const PUNTAJE_MAXIMO = 850;
const dataPuntaje = [
  { name: 'Puntaje', value: PUNTAJE_CREDITICIO },
  { name: 'Restante', value: PUNTAJE_MAXIMO - PUNTAJE_CREDITICIO },
];
...
const AdminDashboard = () => {
 ...
 // --- CÁLCULOS DINÁMICOS CORREGIDOS ---
  ...
  const handleOpenDetail = (creditoId) => {
    ...
    if (!credito) {
      alert("No se pudo encontrar la información del crédito.");
      return;
    }
  ...
  return (
    <>
      {/* ... (Toda la parte superior del dashboard no cambia) ... */}
      ...
      <Card className="mb-6"><CardHeader className="bg-slate-800 text-white rounded-t-lg"><CardTitle>Resumen de Crédito</CardTitle></CardHeader><CardContent className="grid md:grid-cols-2 gap-6 p-6"><div><h3 className="font-semibold mb-2">Utilización de Crédito</h3><Progress value={porcentajeUtilizacion} className="h-3" /><div className="flex justify-between text-sm text-muted-foreground mt-2"><span>Utilizado</span><span>{porcentajeUtilizacion.toFixed(0)}%</span></div><div className="flex justify-between font-bold mt-4"><span>Crédito Total</span><span>${montoTotalOtorgado.toLocaleString()}</span></div><div className="flex justify-between font-bold"><span>Disponible</span><span>${saldoTotalDisponible.toLocaleString()}</span></div></div><div className="flex flex-col items-center justify-center"><h3 className="font-semibold mb-2">Puntaje Crediticio</h3><div style={{ width: '100%', height: 150 }}><ResponsiveContainer><PieChart><Pie data={dataPuntaje} cx="50%" cy="50%" startAngle={180} endAngle={0} innerRadius={60} outerRadius={80} cornerRadius={10} paddingAngle={2} dataKey="value">{dataPuntaje.map((entry, index) => (<Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />))}</Pie></PieChart></ResponsiveContainer><div className="text-3xl font-bold -mt-24 text-center text-green-500">{PUNTAJE_CREDITICIO}</div><div className="text-sm text-muted-foreground text-center">de {PUNTAJE_MAXIMO}</div></div><p className="text-sm font-medium mt-12">Excelente puntaje crediticio</p></div></CardContent></Card>

      {/* --- SECCIÓN DE LISTAS INFERIORES (MODIFICADA) --- */}
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        
        {/* Últimos Créditos */}
        <Card className="lg:col-span-1">
          <CardHeader className="flex flex-row items-center justify-between">
            <CardTitle>Últimos Créditos</CardTitle>
            {/* --- ENLACE MODIFICADO --- */}
            <Link to="/admin/creditos/todos" className="text-sm font-medium text-blue-600 hover:underline">
              Ver todos
            </Link>
          </CardHeader>
          <CardContent>
            {creditosRecientes.map(cred => (
              <button key={cred.id} onClick={() => handleOpenDetail(cred.id)} className="w-full text-left p-3 -mx-3 hover:bg-gray-100 rounded-md transition-colors">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="font-medium">{clientes.find(c=>c.id === cred.clienteId)?.nombre || 'N/A'}</p>
                    <p className="text-sm text-muted-foreground">Monto: ${cred.montoAprobado.toLocaleString()} | Estado: <span className={cred.estado === 'activo' ? 'text-green-500' : 'text-yellow-500'}>{cred.estado}</span></p>
                  </div>
                  <span className="text-sm font-medium text-blue-600">Ver →</span>
                </div>
              </button>
            ))}
          </CardContent>
        ...
        <Card className="lg:col-span-1">
          <CardHeader className="flex flex-row items-center justify-between">
            ...
            {/* --- ENLACE MODIFICADO --- */}
            <Link to="/admin/consumos/todos" className="text-sm font-medium text-blue-600 hover:underline">
              Ver todos
            </Link>
          </CardHeader>
          <CardContent>
            {consumosRecientesConNombre.map(cons => (
              <button key={cons.id} onClick={() => handleOpenDetail(cons.creditoId)} className="w-full text-left p-3 -mx-3 hover:bg-gray-100 rounded-md transition-colors">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="font-medium">{cons.nombreCliente}</p>
                    <p className="text-sm text-muted-foreground">Monto: ${cons.monto.toLocaleString()} | {cons.descripcion || 'Sin descripción'}</p>
                  </div>
                  <span className="text-sm font-medium text-blue-600">Ver →</span>
                </div>
              </button>
        ...
        <Card className="lg:col-span-1">
          <CardHeader className="flex flex-row items-center justify-between">
            ...
            {/* --- ENLACE MODIFICADO --- */}
            <Link to="/admin/pagos/todos" className="text-sm font-medium text-blue-600 hover:underline">
              Ver todos
            </Link>
          </CardHeader>
          <CardContent>
            {pagosRecientes.length > 0 ? (pagosRecientes.map(pago => {
              const credito = creditos.find(c => c.id === pago.creditoId);
              const cliente = clientes.find(cl => cl.id === credito?.clienteId);
              return (
                <button key={pago.id} onClick={() => handleOpenDetail(pago.creditoId)} className="w-full text-left p-3 -mx-3 hover:bg-gray-100 rounded-md transition-colors">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="font-medium">{cliente?.nombre || 'N/A'}</p>
                      <p className="text-sm text-green-600 font-medium">+ ${pago.montoTotal.toLocaleString()}</p>
                    </div>
                    <span className="text-sm font-medium text-blue-600">Ver →</span>
                  </div>
                </button>
              );
            })) : (<p className="text-center py-8 text-gray-500">No hay pagos recientes.</p>)}
          </CardContent>
        </Card>
      </div>
        ...
        <DialogContent className="max-w-2xl">
          {detailData && (
            <div className="printable-area">
              <DialogHeader>
                <DialogTitle className="text-2xl">{detailData.cliente?.nombre}</DialogTitle>
                <DialogDescription>Cédula: {detailData.cliente?.cedula || 'N/A'}</DialogDescription>
              </DialogHeader>
              {detailData.credito ? (
                <div className="py-4 space-y-6">
                  <div>
                    <h4 className="font-semibold mb-2 text-lg">Resumen del Crédito</h4>
                    <div className="p-4 border rounded-lg grid grid-cols-2 gap-4 bg-gray-50">
                  ...
                  <div>
                    <h4 className="font-semibold mb-2 text-lg">Historial de Consumos</h4>
                   ...
                   <div>
                    <h4 className="font-semibold mb-2 text-lg">Historial de Pagos</h4>
              ...
              <DialogFooter className="no-print pt-4 border-t">
                <Button variant="outline" onClick={handlePrintReport}><Printer className="mr-2 h-4 w-4" />Imprimir Resumen</Button>
              </DialogFooter>
            </div>
          )}
        </DialogContent>
